...
class DietaController {
  CriarDieta(req, res) {
    try {
      ...
      if (!Array.isArray(refeicoes) || refeicoes.length <= 0) {
        return res.status(400).json({ erro: "Informe ao menos uma refeicao" });
      }

      for (const refeicao of refeicoes) {
        if (!refeicao.nome_refeicao) {
          return res.status(400).json({ erro: "Cada refeicao deve ter nome_refeicao" });
        }
      }
  ...
  ListarDietas(req, res) {
    try {
      ...
      const idClienteFiltro = req.query.id_cliente ? Number(req.query.id_cliente) : null;

      let idClienteConsulta = null;

      if (role === "cliente") {
        idClienteConsulta = idUsuario;
      }

      if (role === "nutricionista" && idClienteFiltro) {
        idClienteConsulta = idClienteFiltro;
      }

      if (!idClienteConsulta) {
        return res.status(400).json({ erro: "Informe id_cliente para listar dietas" });
      }

      let queryDietas = "SELECT * FROM dietas WHERE id_cliente = ?";
      const paramsDietas = [idClienteConsulta];

      if (role === "nutricionista") {
        queryDietas += " AND id_nutricionista = ?";
        paramsDietas.push(idUsuario);
      }

      queryDietas += " ORDER BY data_inicio DESC";

      conn.execute(queryDietas, paramsDietas, (dietasError, dietasRows) => {
        if (dietasError) return res.status(500).json({ erro: dietasError });
